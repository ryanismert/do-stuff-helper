...
**Revisit waypoint statuses.** The current status names mix past-tense adjectives (`designed`) with present-tense verbs (`implementing`, `waiting`). Consider making them consistent — either all adjectives or all verbs. Also review whether the set of statuses is complete and well-named.
